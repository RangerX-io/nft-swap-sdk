...
// sample transaction on goerli: https://goerli.etherscan.io/tx/0x53b2f046fc8fbffb6e7a92047f462689097e984c0f56880dba65afb9a8a4a841
// sample transaction on goerli(with fee): https://goerli.etherscan.io/tx/0xccdef383de24b28196310116c9021d33f86d352180dc7c075913880cf9514de0
...
describe('NFTSwapV4', () => {
  it('utility functions on class work properly with erc721 sell order', async () => {
    ...
    // onchain order
    // const onchainOrderTx = await nftSwapSdk.presignOrder(order);
    // await onchainOrderTx.tx.wait()

    const onchainOrderStatus = await nftSwapSdk.getOrderStatus(order);
    console.log({ onchainOrderStatus })

    // hosted order
    // const postedOrder = await nftSwapSdk.postOrder(signedOrder, CHAIN_ID.toString());
    // console.log({ postedOrder })

    // const fetchedOrder = await nftSwapSdk.getOrders({ nonce: signedOrder.nonce.toString() })
    // console.log({ fetchedOrder: fetchedOrder.orders })

    // const canceledOrder = await nftSwapSdk.cancelOrder(signedOrder.nonce, 'ERC721');
